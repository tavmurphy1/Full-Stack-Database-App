...
const updateMovie = async (req, res) => {
  ...
  try {
    ...
    if (!lodash.isEqual(newMovie, oldMovie)) {
      const query =
        "UPDATE Movies SET movie_title=?, movie_length=?, movie_total_view=? WHERE id=?";
...
const deleteMovie = async (req, res) => {
  ...
  try {
    ...
    const [response1] = await db.query(
      "DELETE FROM Movies_Actors WHERE pid = ?",
      [movieID]
    );
    ...
    const [response2] = await db.query(
      "DELETE FROM Movies_Directors WHERE pid = ?",
      [movieID]
    );
    ...
    const [response3] = await db.query(
      "DELETE FROM Movies_Genres WHERE pid = ?",
      [movieID]
    );
